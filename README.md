...
- 목표는 실시간 진화 중에도 만들 수 있는 4×4 이하의 작은 패턴 16종에서 라운드마다 선택
  — 정물(블록·벌집·빵·보트·튜브·연못·배·긴 보트·거룻배·뱀·항공모함) · 진동(깜빡이·두꺼비·비컨) · 글라이더 · 이터.
  목록은 [challenge/src/main.rs](challenge/src/main.rs)의 `TARGET_POOL` 이고, 웹 리더보드([web/src/round.rs](web/src/round.rs)의 `POOL`)와
  **순서까지 같아야** 같은 라운드에 같은 모양이 나옵니다 (`cargo test -p challenge` 가 대조합니다).
